...
class FeatureEncoder(torch.nn.Module):
    ...
    def __init__(self, dim_in):
        ...
        if cfg.dataset.node_encoder:
            logger.info(f"Using node encoder {cfg.dataset.node_encoder_name}")
        ...
        if cfg.dataset.edge_encoder:
            logger.info(f"Using edge encoder {cfg.dataset.edge_encoder_name}")
...
class GritTransformer(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        if cfg.posenc_RRWP.enable:
            self.rrwp_abs_encoder = register.node_encoder_dict["rrwp_linear"](cfg.posenc_RRWP.ksteps, cfg.gnn.dim_inner)
            rel_pe_dim = cfg.posenc_RRWP.ksteps
            self.rrwp_rel_encoder = register.edge_encoder_dict["rrwp_linear"] (rel_pe_dim, cfg.gnn.dim_edge, 
        ...
        for l in range(cfg.gt.layers):
            layers.append(TransformerLayer(in_dim=cfg.gt.dim_hidden,
        ...
        logger.info(f"Using head {GNNHead}")
