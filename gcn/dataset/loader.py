        ...
        if self.dataset == 'zinc_standard_agent':
                ...
                if rdkit_mol != None:
                    ...
                    if 50 <= mw <= 900:
                        ...
                        if inchi != None and inchi not in downstream_inchi_set:
                            data = mol_to_graph_data_obj_simple(rdkit_mol)  # manually add mol id
                            data.id = torch.tensor([i])  # id here is the index of the mol in the dataset
                            data.y = torch.tensor(labels[i, :])  # fold information
                            if i in folds[0]:
            ...
            for i in tqdm(range(len(smiles_list))):
                ...
                if rdkit_mol != None:
                    data = mol_to_graph_data_obj_simple(rdkit_mol)
                    # manually add mol id
                    id = torch.tensor([i])  # id here is the index of the mol in the dataset
                    data.y = torch.tensor(labels[i, :])
                    data_list.append(data)
                    data_smiles_list.append(smiles_list[i])
