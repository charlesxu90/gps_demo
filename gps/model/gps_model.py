...
class GPSLayer(nn.Module):
    ...
    def __init__(self, dim_h,
        ...
        if log_attn_weights and global_model_type not in ['Transformer', 'BiasedTransformer']:
            raise NotImplementedError(f"Logging of attention weights is not supported "
        ...
        # MPNNs without edge attributes support.
            ...
            self.local_model = gnn.GCNConv(dim_h, dim_h)
        elif local_gnn_type == 'GIN':
            self.local_gnn_with_edge_attr = False
            gin_nn = nn.Sequential(Linear_pyg(dim_h, dim_h), self.activation(), Linear_pyg(dim_h, dim_h))
            self.local_model = gnn.GINConv(gin_nn)

        # MPNNs supporting also edge attributes.
        elif local_gnn_type == 'GENConv':
            self.local_model = gnn.GENConv(dim_h, dim_h)
        elif local_gnn_type == 'GINE':
            gin_nn = nn.Sequential(Linear_pyg(dim_h, dim_h), self.activation(), Linear_pyg(dim_h, dim_h))
            if self.equivstable_pe:  # Use specialised GINE layer for EquivStableLapPE.
                self.local_model = GINEConvESLapPE(gin_nn)
            else:
                self.local_model = gnn.GINEConv(gin_nn)
        elif local_gnn_type == 'GAT':
            self.local_model = gnn.GATConv(in_channels=dim_h, out_channels=dim_h // num_heads, heads=num_heads, edge_dim=dim_h)
            ...
            self.local_model = gnn.PNAConv(dim_h, dim_h, aggregators=aggregators, scalers=scalers, deg=deg, edge_dim=min(128, dim_h),
                                           towers=1, pre_layers=1, post_layers=1, divide_input=False)
        elif local_gnn_type == 'CustomGatedGCN':
            self.local_model = GatedGCNLayer(dim_h, dim_h, dropout=dropout, residual=True, act=act, equivstable_pe=equivstable_pe)
        else:
        ...
        if self.layer_norm:
            ...
            # self.norm1_local = gnn.norm.GraphNorm(dim_h)
            # self.norm1_attn = gnn.norm.GraphNorm(dim_h)
            # self.norm1_local = gnn.norm.InstanceNorm(dim_h)
            # self.norm1_attn = gnn.norm.InstanceNorm(dim_h)
        if self.batch_norm:
        ...
        if self.layer_norm:
            ...
            # self.norm2 = gnn.norm.GraphNorm(dim_h)
            # self.norm2 = gnn.norm.InstanceNorm(dim_h)
        if self.batch_norm:
    ...
    def forward(self, batch):
        ...
        if self.local_model is not None:
            self.local_model: gnn.conv.MessagePassing  # Typing hint.
    ...
    def extra_repr(self):
        s = f'summary: dim_h={self.dim_h}, local_gnn_type={self.local_gnn_type}, ' \
...
class GPSModel(torch.nn.Module):
    ...
    def __init__(self, dim_in, dim_out):
        ...
        for _ in range(cfg.gt.layers):
            layers.append(GPSLayer(dim_h=cfg.gt.dim_hidden,
