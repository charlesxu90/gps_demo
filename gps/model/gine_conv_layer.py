...
class GINEConvESLapPE(gnn.conv.MessagePassing):
    ...
    def __init__(self, nn, eps=0., train_eps=False, edge_dim=None, **kwargs):
        ...
        if edge_dim is not None:
            ...
            self.lin = gnn.Linear(edge_dim, in_channels)
        else:
            self.lin = None
        ...
        # ICLR 2022 https://openreview.net/pdf?id=e95i1IHcWj
        self.mlp_r_ij = torch.nn.Sequential(torch.nn.Linear(1, out_dim), torch.nn.ReLU(),
                                            torch.nn.Linear(out_dim, 1), torch.nn.Sigmoid())

    def reset_parameters(self):
        gnn.inits.reset(self.nn)
        self.eps.data.fill_(self.initial_eps)
        if self.lin is not None:
            self.lin.reset_parameters()
        gnn.inits.reset(self.mlp_r_ij)
    ...
    def forward(self, x, edge_index, edge_attr=None, pe_LapPE=None, size=None):
        ...
        out = self.propagate(edge_index, x=x, edge_attr=edge_attr, PE=pe_LapPE, size=size)
    ...
    def message(self, x_j, edge_attr, PE_i, PE_j):
        if self.lin is None and x_j.size(-1) != edge_attr.size(-1):
            raise ValueError("Node and edge feature dimensionalities do not match. Consider setting the 'edge_dim' "
...
class GINEConvLayer(nn.Module):
    ...
    def __init__(self, dim_in, dim_out, dropout, residual):
        ...
        gin_nn = nn.Sequential(gnn.Linear(dim_in, dim_out), nn.ReLU(),
                               gnn.Linear(dim_out, dim_out))
        self.model = gnn.GINEConv(gin_nn)
