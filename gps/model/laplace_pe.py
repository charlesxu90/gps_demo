...
class LapPENodeEncoder(torch.nn.Module):
    ...
    def __init__(self, dim_emb, expand_x=True, dim_in=9, dim_pe=16, model_type='DeepSet',
                 n_layers=2, n_heads=4, post_n_layers=1, max_freqs=False, norm_type='none',
                 pass_as_var=False):
        """
        Args:
            dim_emb: Size of final node embedding
            expand_x: Expand node features `x` from dim_in to (dim_emb - dim_pe)
            dim_in: Size of original node features
            dim_pe: Size of Laplace PE embedding
            model_type: Encoder NN model type for PEs
            ...
            max_freqs: Num. eigenvectors (frequencies) to use
            norm_type: Raw PE normalization layer type
            pass_as_var: Pass PE also as a separate variable
        ...
        self.raw_norm = nn.BatchNorm1d(max_freqs) if norm_type == 'batchnorm' else None

        activation = nn.ReLU  # register.act_dict[cfg.gnn.act]
        if model_type == 'Transformer':  # Transformer model for LapPE
